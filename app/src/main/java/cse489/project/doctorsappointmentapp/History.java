...
//    });
    ...
    private void displayAppointmentsWithZeroValue() {
        db.collection("appointments").whereEqualTo("p_id", user.getUid()).whereEqualTo("passed", "1").get().addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
            @Override
    ...
    private void showCancelDialog(final Context context, final String date, final String time) {
        ...
        btnCancel.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                db.collection("appointments").whereEqualTo("date", date).whereEqualTo("time", time).get().addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
                    @Override
                    public void onComplete(@NonNull Task<QuerySnapshot> task) {
                        if (task.isSuccessful()) {
                            for (QueryDocumentSnapshot document : task.getResult()) {
